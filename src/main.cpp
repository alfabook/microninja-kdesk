...
int main(int argc, char *argv[])
{
  ...
  bool test_mode = false, wallpaper_mode = false, mute_mode = false;
  int c;

  // Collect command-line parameters
  while ((c = getopt(argc, argv, "?htwrmia:vq")) != EOF)
    {
      switch (c)
        {
	case '?':
	case 'h':
          cout << "microninja-desktop [ -h | -t | -w | -r | -m | -a <icon name> | -q ]" << endl;
	  ...
	  cout << " -i refresh desktop icons only and exit" << endl;
          cout << " -m mute mode at startup" << endl;
